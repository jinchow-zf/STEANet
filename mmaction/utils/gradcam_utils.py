...
class GradCAM:
    ...
    def _calculate_localization_map(self, inputs, use_labels, delta=1e-20):
        ...
        gradients = self.target_gradients  # [32, 197, 768]
        gradients = gradients[:, 1:, :]  # [32, 196, 768]
        gradients = gradients.transpose(0, 2).transpose(1, 2)  # [768, 32, 196]
        gradients = gradients.reshape(768, 16, 14, 14)
        gradients = gradients.unsqueeze(0)

        activations = self.target_activations  # [197, 32, 768]
        activations = activations[1:, :, :]  # [196, 32, 768]
        activations = activations.transpose(0, 2)  # [768, 32, 196]
        activations = activations.reshape(768, 16, 14, 14)
    ...
    def __call__(self, inputs, use_labels=False, alpha=0.6):
